...
def restore_main_window():
    ...
    # Set the geometry and title for the main window
    window.geometry("500x281")
    window.title("GEOROCK ANALYZER")

    # Create a new canvas in the window
    canvas = tk.Canvas(window, bg="#A8EEAF", height=281, width=500, bd=0, highlightthickness=0, relief="ridge")
    canvas.grid(row=0, column=0, sticky="nsew")  # Use grid instead of place

    # Configure row and column to expand with the window
    window.rowconfigure(0, weight=1)
    window.columnconfigure(0, weight=1)

    # Load and place the image on the canvas
    image_image_1 = PhotoImage(file=relative_to_assets("image_1.png"))
    canvas.create_image(250.0, 97.0, image=image_image_1)
...
def open_rock_list_window():
    ...
    window.geometry("300x400")  # Set the fixed size you want (width x height)
    
    # Add a label to the main window
    label = tk.Label(window, text="Available Georocks", font=("Arial", 14))
    label.grid(row=0, column=0, sticky="ew")  # Use grid
    ...
    # Configure window to expand the frame
    ...
    # Create a list of frames to hold each column of georocks
    columns = []

    # Create the first column frame
    current_column = tk.Frame(content_frame)
    columns.append(current_column)

    # Add georocks as bullet points using labels
    item_count = 0
    current_column_index = 0

    for rock in rock_list:
        # If the current column has reached the max number of items, create a new column
        if item_count >= max_items_per_column:
            current_column_index += 1
            current_column = tk.Frame(content_frame)
            columns.append(current_column)
            item_count = 0

        # Create a label for each georock with a bullet point in front of the name
        bullet_label = tk.Label(current_column, text=f"• {rock}", font=("Arial", 12), justify="left")
        bullet_label.pack(anchor="w", padx=20)  # Set the text alignment to justify

        # Increment the item count
        item_count += 1

    # Pack each column frame in the content frame
    for i, column in enumerate(columns):
        # Center the column within the content frame
        column.pack(side="left", expand=True, fill="both")

    # Update the canvas scroll region after placing all the content
    content_frame.update_idletasks()
    canvas.config(scrollregion=canvas.bbox("all"))

    # Create and pack the back button at the top-left corner of the window
...
def rock_database():
    ...
    class SimpleRockDatabase(RockDatabase):
        ...
        def search(self, physical_characteristics):
            ...
            for rock, characteristics in self.database.items():
                ...
                # Compare hardness
                if characteristics.get("hardness") != user_hardness:
                    match = False
    ...
    class GeoRockApp:
        ...
        def create_main_content(self):
            ...
            # Create and pack the back button at the top-left corner of the window
            back_button = tk.Button(window, text="Back", command=restore_main_window)
            back_button.pack(side="left", anchor="sw", padx=5, pady=5)
        ...
        def search_button_click(self):
            ...
            # Convert hardness input to integer and validate range
            try:
                hardness_value = int(hardness_input)
                if hardness_value < 1 or hardness_value > 10:
                    self.message_text.set("Error: Hardness must be an integer between 1 and 10.")
                    return
            except ValueError:
                self.message_text.set("Error: Hardness must be an integer between 1 and 10.")
                return
        ...
        def search_again(self):
            ...
            self.hardness_label.pack(anchor='center', pady=0)
            self.hardness_entry.pack(anchor='center', pady=0)
            self.search_button.pack(anchor='center', pady=0)
...
# Create a simple label as content for demonstration
label = tk.Label(window, text="GEOROCK ANALYZER")
label.pack(pady=20)
...
canvas.create_text(123.0, 23.0, anchor="nw", text="GEOROCK ANALYZER", fill="#000000", font=("Viga Regular", 26 * -1))
canvas.create_text(170.0, 87.0, anchor="nw", text="What would you like to do?", fill="#000000", font=("Lexend Regular", 12 * -1))
canvas.create_text(202.0, 130.0, anchor="nw", text="PLEASE SELECT", fill="#000000", font=("Lexend Regular", 12 * -1))

# Create and place button 1
button_image_1 = PhotoImage(file=relative_to_assets("button_1.png"))
button_1 = tk.Button(image=button_image_1, borderwidth=0, highlightthickness=0, command=rock_database, relief="flat")
button_1.grid(row=1, column=1, sticky="nsew")  # Use grid instead of place
button_1.image = button_image_1  # Retain reference to the image

# Create and place button 2
button_image_2 = PhotoImage(file=relative_to_assets("button_2.png"))
button_2 = tk.Button(image=button_image_2, borderwidth=0, highlightthickness=0, command=open_rock_list_window, relief="flat")
button_2.grid(row=1, column=0, sticky="nsew")  # Use grid instead of place
